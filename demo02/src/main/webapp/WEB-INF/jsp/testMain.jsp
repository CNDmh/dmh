...
layui.use(['table','form','jquery'], function(){
  ...
  table.on('tool(test)', function(obj){
    ...
    //console.log(obj)
    if(obj.event === 'del'){
      layer.confirm('真的删除行么', function(index){//点击确认按钮后调用此函数
    	//向服务端发送删除指令， 在这里可以使用 Ajax 异步
    	//layui.jquery 找到 jQuery
    	
	layui.jquery.post("book/delete", //提交的 URL
					{
						id : data.id
					//传递参数
					}, function(ret) {
						if (ret.code == '1') {//说明删除成功
						...
						} else if (ret.code == '-1') {//说明删除失败
							layer.alert(ret.msg,//删除失败的消息
							{
								icon : 2
							}, function() {
								layer.close(index);
